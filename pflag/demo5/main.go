...
func main() {
	...
	goflag.String("goname", "go", "go name for application")
	flagset.AddGoFlagSet(goflag.CommandLine)

	testflagset := pflag.NewFlagSet("test", pflag.ExitOnError)
	testflagset.StringP("test", "t", "", "test name for application")
	flagset.AddFlagSet(testflagset)

	if err := flagset.MarkDeprecated("goname", "Use --name instead"); err != nil {
		panic(err)
	}

	flagset.MarkHidden("test")
